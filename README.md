...
| `GET /api/export/ini` | Download current results as an [osstats](https://github.com/Redislabs-Solution-Architects/osstats)-compatible `config.ini` — one `[host:port]` section per result with `host`/`port`/`tls` filled in; `username`/`password` are always blank. |
| `GET /api/export/xlsx` | Download current results as an `.xlsx` shaped like osstats' own output (sheet `ClusterData`, same column layout) — populated only with fields Redis Scanner's single probe knows; osstats' throughput/command-stats columns are omitted, not faked. |
